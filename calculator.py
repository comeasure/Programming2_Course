...
def factor(wtok):
    ...
    if wtok.get_current() == '(':
        ...
        raise SyntaxException("Expected number, function name, '-' or '('")
...
def function_handler(wtok):
    ...
    try:
        ...
        raise EvaluationException(f"Illegal argument to function '{name}({arg})'")
...
def run(line=''):
    try:
        ...
        print(f'*** Error: {e.arg}. At {wtok.get_current()} just after 
{wtok.get_previous()}')
        return 'Error'
